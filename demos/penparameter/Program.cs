 ...
 * (deprecated !) Create pen entity and use it 
 * 
 * (더이상 사용되지 않음 !!!) 사용자가 직접 펜 개체를 생성하는 방식은 더이상 필요하지 않습니다.
 * 사용자가 직접 펜 개체를 생성하고 이를 레이어에 삽입하는 방식은 구 버전에서 지원되던 방식입니다.
 * (신규 방식) 최신 버전에서는 편집기에서 자동으로 펜 집합 기능이 활성화 되어 있어, 약 10여개의 미리 생성된 펜 집합을 제공해 줍니다.
    ...
    class Program
    {
        ...
        private static bool DrawForFieldCorrection(ILaser laser, IRtc rtc, IDocument doc)
        {
            ...
            var markerArg = new MarkerArgDefault()
            {
                ...
                Rtc = rtc,
                Laser = laser,
                IsEnablePens = false, //사용자가 직접 펜 개체를 생성하여 레이어어 삽입하는 방식을 사용할 경우에는 펜 집합 기능이 비활성화 되어 있어야 한다
            };
